...
fn test_profile_save_and_load() {
    ...
    // Verify file exists
    assert!(profile_path.exists(), "Profile file should exist after saving");
...
fn test_profile_with_progress() {
    ...
    // Load it back
    let loaded_plan = load_learning_plan(&profile_path).expect("Failed to load profile with progress");
